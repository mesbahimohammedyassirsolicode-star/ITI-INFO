...
import { getLocalized, getLocalizedArray } from '../utils/i18nHelper';

const FormationCard = ({ formation }) => {
  const { t, i18n } = useTranslation();
  const currentLang = i18n.language || 'fr';

  const title = getLocalized(formation.title, currentLang);
  const duration = getLocalized(formation.duration, currentLang);
  const description = getLocalized(formation.description, currentLang);
  const categoryName = getLocalized(formation.subCategory || formation.category?.name, currentLang);
  const opportunities = getLocalizedArray(formation.opportunities, currentLang);
  ...
  return (
    ...
    >
      ...
      <div className="p-8 flex-grow">
        <h3 className="font-headline-md text-primary mb-2 min-h-[3.5rem] line-clamp-2">{title}</h3>
        ...
        {opportunities && opportunities.length > 0 && (
          <div className="space-y-4">
            <h4 className="font-label-md text-[10px] text-outline uppercase tracking-widest">{t('formations.careerOpportunities')}</h4>
            <div className="flex flex-wrap gap-2">
              ...
              {opportunities.length > 3 && (
                <span className="text-[10px] text-outline italic">{t('formations.moreOpportunities', { count: opportunities.length - 3 })}</span>
              )}
            </div>
          </div>
        )}
